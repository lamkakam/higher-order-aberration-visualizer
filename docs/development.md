...
Pushes to `main`, pull requests, and manual runs run checks only and cannot deploy either hosting target. Markdown-only pull requests, meaning changes where every file matches `**/*.md`, run a lightweight `Quality gates` check that skips full CI. Markdown-only pushes to `main` remain ignored.
