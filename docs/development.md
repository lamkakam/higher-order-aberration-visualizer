...
GitHub Actions uses one required check named `Quality gates` (job `ci`) in [`.github/workflows/ci-cd.yml`](../.github/workflows/ci-cd.yml) only for pull requests targeting `main` or branches matching `v*`. Full CI runs `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, and `npm run e2e` with Node.js 24, Python 3.11, npm caching, and Playwright Chromium installed.
...
Branch pushes, including merges into `main` or `v*` branches, do not trigger CI/CD, and the workflow has no manual dispatch option. Pull requests run checks only and cannot deploy either hosting target. The branch ruleset targets the default branch and `v*` branches, requiring the same `Quality gates` check.
