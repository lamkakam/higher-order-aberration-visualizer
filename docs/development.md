...
GitHub Actions uses one required check named `Quality gates` (job `ci`) in [`.github/workflows/ci-cd.yml`](../.github/workflows/ci-cd.yml) for pull requests targeting `main` or branches matching `v*`, pushes to those branches, and manual `workflow_dispatch` runs. Full CI runs `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, and `npm run e2e` with Node.js 24, Python 3.11, npm caching, and Playwright Chromium installed.

Every pull request to those branches creates the same `Quality gates` job. After checkout with `fetch-depth: 2`, [`scripts/detect-docs-only.mjs`](../scripts/detect-docs-only.mjs) compares the PR merge commit with its first parent using `git diff --name-only --no-renames -z HEAD^1 HEAD`. A nonempty diff containing only paths ending in `.md`, at the root or in nested directories, skips environment setup, dependency installation, and full CI, prints a short explanation, and passes the gate. The comparison includes all PR changes, uses null-delimited paths, and treats renames as deletions and additions, so renaming code to Markdown still runs full CI. Empty diffs run full CI, and detection errors fail the job.

Markdown-only pushes to `main` or `v*` branches remain ignored. Manual runs always execute full CI. Branch pushes, pull requests, and manual runs run checks only and cannot deploy either hosting target. The branch ruleset targets the default branch and `v*` branches, requiring the same `Quality gates` check.
