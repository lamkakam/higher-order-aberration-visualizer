...
GitHub Actions uses one required check named `Quality gates` (job `ci`) in [`.github/workflows/ci-cd.yml`](../.github/workflows/ci-cd.yml) for pull requests targeting `main`. The workflow also matches pull requests targeting `v*` branches; this repository does not use such branches. Full CI runs `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, and `npm run e2e` with Node.js 24, Python 3.11, npm caching, and Playwright Chromium installed.

Every matching pull request creates the same `Quality gates` job. After checkout with `fetch-depth: 2`, [`scripts/detect-docs-only.mjs`](../scripts/detect-docs-only.mjs) compares the PR merge commit with its first parent using `git diff --name-only --no-renames -z HEAD^1 HEAD`. A nonempty diff containing only paths ending in `.md`, at the root or in nested directories, skips environment setup, dependency installation, and full CI, prints a short explanation, and passes the gate. The comparison includes all PR changes, uses null-delimited paths, and treats renames as deletions and additions, so renaming code to Markdown still runs full CI. Empty diffs run full CI, and detection errors fail the job.

Branch pushes, including merges into `main`, do not trigger CI/CD, and the workflow has no manual dispatch option. Pull requests run checks only and cannot deploy either hosting target. The branch ruleset targets only `main` and requires the `Quality gates` check. Version tags are governed by separate tag rulesets.

Pushing a tag that matches `v*` runs the release workflow with the same quality gates. It first builds root-based `dist`, packages that build as a zip archive, and prepares the same build as the `cloudflare-pages` artifact. It then rebuilds `dist` with `STATIC_SUBPATH_DEPLOYMENT=true` and uploads that build as the `github-pages` artifact before creating the GitHub Release with generated release notes and the root-based build attached. After the release job succeeds, dependent environment jobs deploy the host-specific artifacts to GitHub Pages and to the `higher-order-aberration-visualizer` Cloudflare Pages Direct Upload project with Wrangler. The tag promotes its exact commit to both hosts even when that commit is not on `main`; non-version tags, branch pushes, pull requests, and `workflow_dispatch` runs cannot trigger either deployment.
...
[`public/sw.js`](../public/sw.js) registers in both local development and production, and serves supported translation JSON files network-first with a versioned cached fallback. The committed `prysm` wheel remains cache-first. When translation files change for a release, bump the service worker `CACHE_VERSION` so the precached fallback updates too. During local development, edited translation JSON should be fetched from the network when available; stale cached fallback responses can still appear while offline or before the updated service worker activates.
