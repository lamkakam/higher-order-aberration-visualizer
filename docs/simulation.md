...
WebMCP coefficient patches use the same editable Zernike term list and coefficient bounds as the UI. Basic route tool calls patch the shared `550 nm` coefficient map. Advanced route tool calls must include `wavelengthNm` as the JSON number `550`, `656`, or `486`, and patch only that wavelength map even when `Sync wavelengths` is enabled.
