...
WebMCP coefficient patches use the same editable Zernike term list and internal `[-5, 5]` wave bounds as the UI. Both tools require `apertureDiameterMm` (a finite number at least `0.5`), `coefficientUnit` (`wave` or `micron`), and coefficient patches. Wave inputs are stored directly; micron inputs are converted per wavelength with `microns / (wavelengthNm / 1000)` and the converted wave value is range-checked. Basic calls contain one non-empty `coefficients` object and always convert against and patch the shared `550 nm` map. Advanced calls contain a non-empty `coefficientsByWavelength` object whose optional `550`, `656`, and `486` properties are themselves non-empty patches. One call can patch any combination of those wavelengths independently, regardless of `Sync wavelengths`, while preserving omitted values. Every patch and the aperture are validated before any state changes. Supplying a `656` or `486` patch automatically switches Advanced Mode to polychromatic and selects `550 nm` for diagnostics; a `550`-only call leaves spectral mode unchanged.
